    ...
    return (
      <div className="space-y-4">
        ...
        {currentStep.type === "countdown-burst" ? (
          ...
          pendingFile ? (
            <>
              <p className="text-sm text-foreground/70">Is this the right screenshot?</p>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
            ...
            <>
              ...
              {config.useChallenge && challenge && (
                <div className="rounded-lg border border-border bg-background p-3 text-center">
                  <p className="text-xs font-medium uppercase tracking-wide text-foreground/50">Your one-time code</p>
                  <p className="mt-0.5 font-mono text-lg font-semibold tracking-wider">{challenge}</p>
                </div>
              )}
          ...
          <>
            <p className="text-sm text-foreground/70">Is this photo clear and usable?</p>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
    ...
    return (
      <div className="space-y-4">
          ...
          return (
            <div key={step.id}>
              <div className="mb-2 flex items-center justify-between">
                <p className="text-xs font-medium text-foreground/50">{step.label}</p>
                {canEdit && (
                  <button onClick={() => beginEditStep(step.id)} className="text-xs font-medium text-accent">
                    Redo this test
                  </button>
                )}
              </div>
          ...
          return (
            <div key={step.id}>
              <div className="mb-1 flex items-center justify-between">
                <p className="text-xs font-medium text-foreground/50">{s.label}</p>
                {canEdit && (
                  <button onClick={() => beginEditStep(step.id)} className="text-xs font-medium text-accent">
        ...
        {productPhoto && (
          <div>
            <div className="mb-2 flex items-center justify-between">
              <p className="text-xs font-medium text-foreground/50">Photo of the device</p>
              {canEdit && (
                <button onClick={() => setPhase("product-photo")} className="text-xs font-medium text-accent">
                  Retake
                </button>
              )}
            </div>
        ...
        {phase === "review" && (
          <div className="space-y-2">
            ...
            <button onClick={retakeEverything} className="w-full text-center text-xs text-foreground/40 underline">
