...
export default function GoProFlow({ sessionId }: { sessionId: string }) {
  ...
  if (phase === "bluetooth-connecting") {
    ...
    return <p className="text-center text-sm text-ink-secondary">{message}</p>;
  }
  ...
  if (phase === "bluetooth-result") {
    ...
    return (
      <div className="space-y-4">
        ...
        {success ? (
          <>
            <p className="rounded-lg border border-proof/30 bg-proof-wash px-3 py-2 text-sm text-proof">
          ...
          <>
            <p className="text-sm text-ink-secondary">{bt.note}</p>
            <div className="flex gap-2">
              <button
                onClick={retryBluetooth}
                className="flex-1 rounded-lg border border-border-control py-2.5 text-sm font-medium"
  ...
  return (
    <div className="space-y-4">
      ...
      {phase === "camera-ready" && pendingFrame && (
        <div className="space-y-4">
          <p className="text-sm text-ink-secondary">Can you clearly read the GoPro&apos;s battery and storage status?</p>
        ...
        <div className="space-y-4">
          ...
          {frame && (
            <div>
              ...
              <button onClick={retake} className="mt-1.5 text-xs font-medium text-signal">
                Retake status photo
              </button>
            </div>
          )}
